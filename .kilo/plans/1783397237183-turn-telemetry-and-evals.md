...
This plan uses 4 agents across 3 waves to maximize parallelism while respecting file dependencies.

**Task Dependency Graph:**
```
Task 1 (agent/metrics.go)  ───┐
                                ├──► Task 2 (agent/runner.go integration) ──┐
Task 3 (db/turn_metrics)   ───┘                                             ├──► Task 4 (app/app.go wiring)
                                                                            │
                                                                            └──► Task 5 (agent/eval_scenarios_test.go)
```

### Wave 1 — Two parallel agents (no file overlap)

**Agent 1A — Task 1 (Agent metrics types):**
- Creates `internal/agent/metrics.go`
- Creates `internal/agent/metrics_test.go` (pure helper tests only — Steps 1-5)
- Files touched: `internal/agent/metrics.go`, `internal/agent/metrics_test.go`
- Does NOT touch `runner.go` or `runner_test.go`

**Agent 1B — Task 3 (DB turn_metrics table):**
- Modifies `internal/db/migrations.go` (appends table DDL)
- Creates `internal/db/turnmetrics.go`
- Creates `internal/db/turnmetrics_test.go`
- Files touched: `internal/db/migrations.go`, `internal/db/turnmetrics.go`, `internal/db/turnmetrics_test.go`

**Integration step after Wave 1:** Verify both agents' tests pass and full suite is clean:
```bash
CGO_ENABLED=1 go test -count=1 ./internal/agent/ ./internal/db/
```

### Wave 2 — One agent (depends on Task 1's types)

**Agent 2 — Runner integration:**
- Extends `scriptedProvider` in `runner_test.go` with `usages` field (Step 3)
- Adds `MetricsObserver`, `stats`, `statsMu` to `Runner` in `runner.go` (Step 4)
- Wires all collection points in `runner.go` (Step 5)
- Appends Runner integration tests to `metrics_test.go` (Steps 1 append, 6-7)
- Files touched: `internal/agent/runner.go`, `internal/agent/runner_test.go`, `internal/agent/metrics.go`, `internal/agent/metrics_test.go`

**Integration step after Wave 2:** Verify all agent tests pass:
```bash
CGO_ENABLED=1 go test -count=1 ./internal/agent/...
```

### Wave 3 — Two parallel agents (no file overlap)

**Agent 3A — Task 4 (App wiring):**
- Adds `SessionID()` and `Logger()` to `internal/app/session/session.go`
- Adds `metricsRecorder` function and wires both runner constructors in `internal/app/app.go`
- Creates `internal/app/metrics_recorder_test.go`
- Files touched: `internal/app/session/session.go`, `internal/app/app.go`, `internal/app/metrics_recorder_test.go`

**Agent 3B — Task 5 (Eval scenarios):**
- Creates `internal/agent/eval_scenarios_test.go`
- Files touched: `internal/agent/eval_scenarios_test.go`

**Integration step after Wave 3 — Full suite verification:**
```bash
CGO_ENABLED=1 go test -count=1 ./...
gofmt -l .
go vet ./... 2>&1 | grep -v "app.go:.*copies lock value" || true
```
