...
Tool results from earlier in the conversation are in the transcript and context pack.`)
	if workingDir != "" {
		...
		b.WriteString(". Relative paths in tool arguments are resolved from this directory, and shell.run executes with this directory as its cwd. When this session is isolated in a worktree, that is the worktree root: do not prefix paths with .marshal/worktrees/<branch>/ yourself.")
	}
	return b.String()
}
