...
func TestDynamicMaxHeightBudget(t *testing.T) {
	...
	// 40 - transcriptFrameRows(0) - statusLineRows(1) - turnSpinnerRows(0 when
	// idle) - minTranscriptRows(4); no todo panel, live strip, dock, or popup.
...
func TestMaxHeightFloorIsOneRow(t *testing.T) {
	t.Run("height_3_budget_returns_1", func(t *testing.T) {
		m := newViewTestModel(t, 80, 24)
		m.height = 3 // budget = 3 - 0 - 1 - 1 - 0 - 0 - 0 - 1 = 0, floor to 1
	...
	t.Run("height_2_floor_kicks_in", func(t *testing.T) {
		m := newViewTestModel(t, 80, 24)
		m.height = 2 // budget = 2 - 0 - 1 - 1 - 0 - 0 - 0 - 1 = -1, floor to 1
		if got := m.maxInputHeight(); got != 1 {
