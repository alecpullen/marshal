...
func TestApplyConnectDonePreservesExistingProfiles(t *testing.T) {
	...
	m.state.Config.AgentProfiles = map[string]routing.AgentProfile{
		"handcrafted": {Name: "handcrafted", Roles: map[routing.AgentRole]routing.RoleBinding{
			routing.RoleImplementer: {Preset: "mine"},
		}},
	}
	...
	if _, ok := m.state.Config.AgentProfiles["handcrafted"]; !ok {
		t.Error("connecting a model destroyed a user's existing profile")
	}
}
...
func TestDispatchPromptCommandStartsAgentRun(t *testing.T) {
	...
	if !m.busy {
		t.Fatal("prompt command should start an agent run")
	}
	if cmd == nil {
	...
	select {
	case goal := <-runner.called:
		if goal != "Review the current diff.\n\nmain.go" {
			t.Fatalf("goal = %q, want body with args appended", goal)
		}
	case <-time.After(time.Second):
...
func TestDispatchPromptCommandBusySteers(t *testing.T) {
	...
	if !ok {
		t.Fatal("prompt command while busy should enqueue steering")
	}
	if followUp != "Review the current diff." {
