...
func TestValidate_MissingExecutorAPIKey(t *testing.T) {
	...
	err := cfg.Validate()
	if err == nil || err.Error() != "executor.api_key / $FIREWORKS_API_KEY is required" {
		t.Errorf("expected 'executor.api_key / $FIREWORKS_API_KEY is required', got: %v", err)
	}
}
...
func TestValidate_MissingCriticAPIKey(t *testing.T) {
	...
	err := cfg.Validate()
	if err == nil || err.Error() != "critic.api_key / $FIREWORKS_API_KEY is required" {
		t.Errorf("expected 'critic.api_key / $FIREWORKS_API_KEY is required', got: %v", err)
	}
}
