...
// Layer is the interface for git operations.
// This allows the marshal package to use git without depending on the concrete type.
type Layer interface {
	...
	// DiffBranch returns the diff between baseBranch and branch using committed history.
	// Safe to call without affecting the working tree.
	DiffBranch(branch string) (string, error)
}
...
// NewAdapter returns g as a Layer. Git already implements Layer directly,
// so this is a no-op convenience function for callers that need a Layer value.
func NewAdapter(g *Git) Layer {
	return g
}
...
// DiffBranch returns committed changes in branch relative to baseBranch.
// Uses three-dot syntax (git diff base...branch) so it shows only what branch added,
// not divergence from base. Does not touch the working tree.
func (g *Git) DiffBranch(branch string) (string, error) {
	cmd := exec.Command("git", "-C", g.repoRoot, "diff", g.baseBranch+"..."+branch)
	output, err := cmd.CombinedOutput()
	if err != nil {
		return "", &GitError{
			Op:      "diff-branch",
