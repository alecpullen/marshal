...
func TestLoadSkillsBothDirs(t *testing.T) {
	...
	list := idx.List()
	if len(list) != 12 {
		t.Fatalf("List length = %d, want 12 (two filesystem skills + ten built-ins)", len(list))
	}
...
func TestLoadSkillsNeitherDirExists(t *testing.T) {
	...
	if len(idx.List()) != 10 {
		t.Fatalf("List length = %d, want 10 (built-ins only for missing dirs)", len(idx.List()))
	}
...
func TestLoadSkillsOnlyProjectDir(t *testing.T) {
	...
	if len(idx.List()) != 11 {
		t.Fatalf("List length = %d, want 11 (project skill + ten built-ins)", len(idx.List()))
	}
}
...
func TestLoadSkillsSkipsNonMdFiles(t *testing.T) {
	...
	list := idx.List()
	if len(list) != 11 {
		t.Fatalf("List length = %d, want 11 (skill + ten built-ins)", len(list))
	}
...
func TestLoadSkillsMalformedFileSkipped(t *testing.T) {
	...
	list := idx.List()
	if len(list) != 11 {
		t.Fatalf("List length = %d, want 11 (good + ten built-ins)", len(list))
	}
...
func TestLoadSkillsBundleAndFlatCoexist(t *testing.T) {
	...
	if len(idx.List()) != 12 {
		t.Fatalf("List length = %d, want 12 (two skills + ten built-ins)", len(idx.List()))
	}
}
...
func TestLoadSkillsIgnoresDirsWithoutSkillMD(t *testing.T) {
	...
	if len(idx.List()) != 11 {
		t.Fatalf("List length = %d, want 11 (skill + ten built-ins)", len(idx.List()))
	}
}
