...
func TestLoadSkillsBothDirs(t *testing.T) {
	...
	list := idx.List()
	if len(list) != 13 {
		t.Fatalf("List length = %d, want 13 (two filesystem skills + eleven built-ins)", len(list))
	}
...
func TestLoadSkillsNeitherDirExists(t *testing.T) {
	...
	if len(idx.List()) != 11 {
		t.Fatalf("List length = %d, want 11 (built-ins only for missing dirs)", len(idx.List()))
	}
...
func TestLoadSkillsOnlyProjectDir(t *testing.T) {
	...
	if len(idx.List()) != 12 {
		t.Fatalf("List length = %d, want 12 (project skill + eleven built-ins)", len(idx.List()))
	}
}
...
func TestLoadSkillsSkipsNonMdFiles(t *testing.T) {
	...
	list := idx.List()
	if len(list) != 12 {
		t.Fatalf("List length = %d, want 12 (skill + eleven built-ins)", len(list))
	}
...
func TestLoadSkillsMalformedFileSkipped(t *testing.T) {
	...
	list := idx.List()
	if len(list) != 12 {
		t.Fatalf("List length = %d, want 12 (good + eleven built-ins)", len(list))
	}
...
func TestLoadSkillsBundleAndFlatCoexist(t *testing.T) {
	...
	if len(idx.List()) != 13 {
		t.Fatalf("List length = %d, want 13 (two skills + eleven built-ins)", len(idx.List()))
	}
}
...
func TestLoadSkillsIgnoresDirsWithoutSkillMD(t *testing.T) {
	...
	if len(idx.List()) != 12 {
		t.Fatalf("List length = %d, want 12 (skill + eleven built-ins)", len(idx.List()))
	}
}
