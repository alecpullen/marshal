...
func TestParseYAMLFrontmatterValid(t *testing.T) {
	raw := `---
name: systematic-debugging
...
`
	...
	if skill.Description != "Use when encountering any bug, test failure, or unexpected behavior" {
		t.Fatalf("Description = %q", skill.Description)
	}
	if skill.Risk != "read_only" {
		t.Fatalf("Risk = %q, want read_only", skill.Risk)
...
func TestParseYAMLFrontmatterDefaultRisk(t *testing.T) {
	raw := `---
name: my-skill
description: A skill without explicit risk
---
...
func TestParseYAMLFrontmatterQuotedDescription(t *testing.T) {
	raw := `---
name: brainstorming
...
`
	...
	if skill.Name != "brainstorming" {
		t.Fatalf("Name = %q, want brainstorming", skill.Name)
	}
	if skill.Description != "You MUST use this before any creative work" {
		t.Fatalf("Description = %q", skill.Description)
	}
}

func TestParseYAMLFrontmatterMissingName(t *testing.T) {
	raw := `---
description: A skill without a name
---
...
func TestParseYAMLFrontmatterMissingClosing(t *testing.T) {
	raw := `---
name: my-skill
