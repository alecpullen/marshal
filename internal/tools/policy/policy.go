...
func analyzeCommand(cmd string) (guardrailVerdict, error) {
	...
	// Use argv-aware classification (shlex-based) for destructive
	// patterns that the substring guardrailPatterns would miss (e.g.
	// `rm -fr`, `rm -r -f`, `chmod --recursive`). This is additive:
	// ClassifyCommand catches destructive patterns, then the stage loop
	// below catches non-destructive substring patterns (sudo, mkfs) and
	// dynamic-argv0 cases.
