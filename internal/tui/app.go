...
// SubmitTaskMsg is sent when the user submits a message (prompt or composer).
// In agent-centric mode, this starts a conversation, not immediately a task.
type SubmitTaskMsg struct {
	Task string
}

// UserMessageMsg is sent when the user sends a message in conversation mode.
type UserMessageMsg struct {
	Content string
}

// MarshalResponseMsg is sent when the Marshal responds to a user message.
type MarshalResponseMsg struct {
	Response *marshal.MarshalResponse
}

// ConversationModeMsg signals a change in conversation UI mode.
type ConversationModeMsg struct {
...
type ExecCommandMsg struct {
	Name    string
	Args    []string
	CmdType string // "builtin", "shell", "view" (from custom commands)
}
...
type Model struct {
	...
	// Agent-centric conversation state
	conversation *conversation.Conversation
	convMode     conversation.Mode

	loopAdapter *LoopAdapter // for command handlers

	execEndpoint    endpointCfg
	criticEndpoint  endpointCfg
	marshalEndpoint endpointCfg
	ollamaURL       string // for :models and :pull commands
	execConn        connStatus
	criticConn      connStatus
	marshalConn     connStatus

	cfg   *config.Config // full config for custom commands
	store *store.Store   // for persistent model preferences
}
...
func New() Model {
	return Model{
		...
		help:         newHelpModel(),
		conversation: conversation.New(conversation.GenerateID()),
		convMode:     conversation.ModeChat,
	}
}
...
func (m Model) WithConfig(cfg *config.Config) Model {
	...
	m.execEndpoint = endpointCfg{url: cfg.Executor.BaseURL, provider: cfg.Executor.GetProvider()}
	m.criticEndpoint = endpointCfg{url: cfg.Critic.BaseURL, provider: cfg.Critic.GetProvider()}
	marshalCfg := cfg.GetMarshalConfig()
	m.marshalEndpoint = endpointCfg{url: marshalCfg.BaseURL, provider: marshalCfg.GetProvider()}
	m.cfg = cfg           // Store for custom command access
	m.main.SetConfig(cfg) // Set config for command completions

	// Set Ollama URL for :models and :pull commands
	// Prefer executor if it uses Ollama, otherwise check marshal
	if strings.EqualFold(cfg.Executor.Provider, "ollama") {
		m.ollamaURL = cfg.Executor.BaseURL
	} else if strings.EqualFold(cfg.Marshal.Provider, "ollama") {
		m.ollamaURL = cfg.Marshal.BaseURL
	} else if strings.EqualFold(cfg.Planner.Provider, "ollama") {
		m.ollamaURL = cfg.Planner.BaseURL
	}
...
func (m Model) Init() tea.Cmd {
	return tea.Batch(
		m.main.Init(),
		doConnCheck(m.execEndpoint, m.criticEndpoint, m.marshalEndpoint),
		doModelReadyCheck(m.cfg),
	)
}
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "?":
			// Only show help on ? when not typing in the main input.
			// When overlay is open or focus is on sidebar, ? works as a shortcut.
			if m.overlay == overlayNone && m.focus != focusMain {
				m.overlay = overlayHelp
				return m, nil
			}
		}

	// Conversation and task lifecycle messages
	case SubmitTaskMsg:
		// In agent-centric mode, submit user message to conversation
		cmds = append(cmds, func() tea.Msg {
			return UserMessageMsg{Content: msg.Task}
		})
		return m, tea.Batch(cmds...)

	case UserMessageMsg:
		// Display user message in conversation
		m.main.AppendConversationMessage("user", msg.Content)
		// Forward to adapter for processing
		if m.loopAdapter != nil {
			adapter := m.loopAdapter
			content := msg.Content
			cmds = append(cmds, func() tea.Msg {
				convID := adapter.GetCurrentConversation()
				if convID == "" {
					convID = adapter.StartConversation()
				}
				adapter.SendConversationMessage(convID, content)
				return nil
			})
		}

	case MarshalResponseMsg:
		// Display Marshal's response (cleaned — think blocks stripped)
		m.main.AppendConversationMessage("marshal", msg.Response.Content)
		// Show think block inline in the same conversation block
		if msg.Response.ThinkContent != "" {
			m.main.SetThinkBlock(msg.Response.ThinkContent)
		}
		// Update conversation state
		m.convMode = msg.Response.Type.Mode()
		// Handle task execution if Marshal decided to execute
		if msg.Response.ExecuteTask != nil && m.loopAdapter != nil {
			taskID := fmt.Sprintf("task-%d", len(m.main.blocks))
			m.loopAdapter.SubmitTask(taskID, *msg.Response.ExecuteTask)
		}
		// Handle clarifying questions
		if msg.Response.Type == conversation.ResponseClarification && len(msg.Response.Questions) > 0 {
			m.convMode = conversation.ModeClarifying
		}
	...
	case connCheckMsg:
		...
		m.marshalConn = msg.marshal
		m.config = m.config.SetConnStatus(msg.exec, msg.critic, msg.marshal)
		// Schedule the next re-check; HTTP runs in the tick goroutine (4s timeout, 30s interval).
		exec, critic, marshal := m.execEndpoint, m.criticEndpoint, m.marshalEndpoint
		return m, tea.Tick(30*time.Second, func(_ time.Time) tea.Msg {
			return doConnCheck(exec, critic, marshal)()
		})

	case modelReadyMsg:
		for _, line := range modelResultLines(msg) {
			m.main.AppendLine(line)
		}
		return m, nil
...
func (m Model) renderStatusbar() string {
	...
	// Marshal is shown first (user-facing), then exec and critic (internal)
	right := connDot(m.marshalConn) + lipgloss.NewStyle().Foreground(colTx3).Render(" marshal") +
		"   " +
		connDot(m.execConn) + lipgloss.NewStyle().Foreground(colTx3).Render(" exec") +
		"   " +
...
func (m Model) execCommand(msg ExecCommandMsg) (Model, tea.Cmd) {
	...
	switch msg.CmdType {
	case "shell":
		return m.execCustomShellCommand(msg.Name, msg.Args)
	case "view":
		return m.execCustomViewCommand(msg.Name, msg.Args)
	case "builtin":
		// For builtin aliases, resolve and fall through to built-in handler
		if cmd, ok := m.cfg.Commands[msg.Name]; ok && cmd.Builtin != "" {
			// This is a custom command aliasing a builtin
			// Redirect to the target builtin
			msg.Name = cmd.Builtin
			msg.CmdType = "builtin"
			msg.Args = msg.Args
	...
	case "models":
		return m.handleModelsCommand()

	case "pull":
		if len(msg.Args) == 0 {
			return m, func() tea.Msg {
				return LogLineMsg{Line: LogLine{Kind: lineError, Content: "Usage: :pull <model>"}}
			}
		}
		return m.handlePullCommand(msg.Args[0])

	case "setmodel":
		if len(msg.Args) < 2 {
			return m, func() tea.Msg {
				return LogLineMsg{Line: LogLine{Kind: lineError, Content: "Usage: :setmodel <role> <model>\nRoles: executor, critic, planner, marshal"}}
			}
		}
		return m.handleSetModelCommand(msg.Args[0], msg.Args[1])
	}

	return m, nil
}

// execCustomShellCommand executes a custom shell command.
func (m Model) execCustomShellCommand(name string, args []string) (Model, tea.Cmd) {
	customCmd, ok := m.cfg.Commands[name]
	if !ok || customCmd.Action != "shell" {
		return m, func() tea.Msg {
			return LogLineMsg{Line: LogLine{Kind: lineError, Content: fmt.Sprintf("unknown shell command: %s", name)}}
		}
	}
	...
	return m, func() tea.Msg {
		...
		if err != nil {
			return LogLineMsg{Line: LogLine{
				Kind:    lineError,
		...
		if outputStr != "" {
			...
			for _, line := range lines {
				m.main.AppendLine(LogLine{Kind: lineSuccess, Content: line})
			}
		}
		return LogLineMsg{Line: LogLine{
			Kind:    lineSuccess,
			Content: fmt.Sprintf("✓ %s completed", name),
		}}
	}
}

// execCustomViewCommand opens a view overlay for custom view commands.
func (m Model) execCustomViewCommand(name string, args []string) (Model, tea.Cmd) {
	customCmd, ok := m.cfg.Commands[name]
	if !ok || customCmd.Action != "view" {
		return m, func() tea.Msg {
			return LogLineMsg{Line: LogLine{Kind: lineError, Content: fmt.Sprintf("unknown view command: %s", name)}}
		}
	}

	// Map view names to overlays
	switch customCmd.View {
	...
	default:
		return m, func() tea.Msg {
			return LogLineMsg{Line: LogLine{
				Kind:    lineError,
				Content: fmt.Sprintf("unknown view: %s (available: sessions, config, help)", customCmd.View),
			}}
		}
	}
}

// handleModelsCommand handles the :models command to list available Ollama models.
func (m Model) handleModelsCommand() (Model, tea.Cmd) {
	if m.ollamaURL == "" {
		return m, func() tea.Msg {
			return LogLineMsg{Line: LogLine{Kind: lineError, Content: "Ollama not configured. Set provider = \"ollama\" in executor, marshal, or planner section."}}
		}
	}

	ollamaBe := backend.NewOllamaBackend("tui-models", m.ollamaURL)
	return m, func() tea.Msg {
		models, err := ollamaBe.ListModels(context.Background())
		if err != nil {
			return LogLineMsg{Line: LogLine{Kind: lineError, Content: fmt.Sprintf("failed to list models: %v", err)}}
		}

		if len(models) == 0 {
			return LogLineMsg{Line: LogLine{Kind: lineSystem, Content: "No models installed. Use :pull <model> to download one."}}
		}

		var cmds []tea.Cmd
		cmds = append(cmds, func() tea.Msg {
			return LogLineMsg{Line: LogLine{Kind: lineSystem, Content: fmt.Sprintf("Available models (%d):", len(models))}}
		})
		for _, model := range models {
			info := model.FormatModelInfo()
			cmds = append(cmds, func() tea.Msg {
				return LogLineMsg{Line: LogLine{Kind: lineSystem, Content: "  " + info}}
			})
		}
...
// handlePullCommand handles the :pull command to download an Ollama model.
func (m Model) handlePullCommand(modelName string) (Model, tea.Cmd) {
	if m.ollamaURL == "" {
		return m, func() tea.Msg {
			return LogLineMsg{Line: LogLine{Kind: lineError, Content: "Ollama not configured. Set provider = \"ollama\" in executor, marshal, or planner section."}}
		}
	...
	ollamaBe := backend.NewOllamaBackend("tui-pull", m.ollamaURL)
	return m, func() tea.Msg {
		// Announce start
		logLine := LogLine{Kind: lineSystem, Content: fmt.Sprintf("Downloading %s...", modelName)}
		m.main.AppendLine(logLine)

		err := ollamaBe.PullModel(context.Background(), modelName, func(progress backend.PullModelProgress) {
			if progress.Total > 0 {
				pct := float64(progress.Completed) / float64(progress.Total) * 100
				m.main.AppendLine(LogLine{Kind: lineSystem, Content: fmt.Sprintf("  %s (%.1f%%)", progress.Status, pct)})
			} else {
				m.main.AppendLine(LogLine{Kind: lineSystem, Content: fmt.Sprintf("  %s", progress.Status)})
			}
		})

		if err != nil {
			return LogLineMsg{Line: LogLine{Kind: lineError, Content: fmt.Sprintf("pull failed: %v", err)}}
		}

		return LogLineMsg{Line: LogLine{Kind: lineSuccess, Content: fmt.Sprintf("✓ %s installed successfully", modelName)}}
	}
}

// handleSetModelCommand handles the :setmodel command to persist a model preference.
func (m Model) handleSetModelCommand(role, modelName string) (Model, tea.Cmd) {
	validRoles := map[string]bool{"executor": true, "critic": true, "planner": true, "marshal": true}
	if !validRoles[role] {
		return m, func() tea.Msg {
			return LogLineMsg{Line: LogLine{Kind: lineError, Content: fmt.Sprintf("invalid role %q — valid: executor, critic, planner, marshal", role)}}
		}
	}

	if m.store == nil {
		return m, func() tea.Msg {
			return LogLineMsg{Line: LogLine{Kind: lineError, Content: "no store available — model preferences require a session database"}}
		}
	}

	// Resolve provider and base URL from current config
	var provider, baseURL string
	switch role {
	case "executor":
		provider = m.cfg.Executor.Provider
		baseURL = m.cfg.Executor.BaseURL
	case "critic":
		provider = m.cfg.Critic.Provider
		baseURL = m.cfg.Critic.BaseURL
	case "planner":
		provider = m.cfg.Planner.Provider
		baseURL = m.cfg.Planner.BaseURL
	case "marshal":
		provider = m.cfg.Executor.Provider
		baseURL = m.cfg.Executor.BaseURL
	}
	...
	err := m.store.SetModelPreference(role, modelName, provider, baseURL)
	if err != nil {
		return m, func() tea.Msg {
			return LogLineMsg{Line: LogLine{Kind: lineError, Content: fmt.Sprintf("failed to save preference: %v", err)}}
		}
	}

	return m, func() tea.Msg {
		return LogLineMsg{Line: LogLine{Kind: lineSuccess, Content: fmt.Sprintf("✓ %s model set to %s", role, modelName)}}
	}
}
