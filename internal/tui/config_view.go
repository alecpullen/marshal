...
func (m ConfigModel) SetConnStatus(exec, critic, marshal connStatus) ConfigModel {
	m.execConn = exec
	m.criticConn = critic
	m.marshalConn = marshal
	return m
}
...
func (m ConfigModel) View(w, h int) string {
	...
	// Title with live connection summary (marshal first as user-facing)
	marshalStatus := connDot(m.marshalConn) + lipgloss.NewStyle().Foreground(colTx3).Render(" marshal")
	...
	titleRight := marshalStatus + "  " + execStatus + "  " + criticStatus
