...
func noopEngine(_ context.Context, _ string, _ loop.Sink, _, _ string, _, _ []string, _ bool) error {
	return nil
}

func newTestModel() model {
	m := newModel(context.Background(), noopGate, noopEngine, skills.New(), nil, "test-session", "/tmp", []string{}, nil, &progRef{}, nil, nil, nil, nil)
...
func TestModel_SlashSkillsList_WithSkills(t *testing.T) {
	...
	m := newModel(context.Background(), noopGate, noopEngine, reg, nil, "test-session", "/tmp", []string{}, nil, &progRef{}, nil, nil, nil, nil)
	next, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	m = next.(model)
...
func TestModel_SlashSkillActivation(t *testing.T) {
	...
	engineFn := func(_ context.Context, _ string, _ loop.Sink, execExtra, _ string, _, _ []string, _ bool) error {
	...
	m := newModel(context.Background(), noopGate, engineFn, reg, nil, "test-session", "/tmp", []string{}, nil, &progRef{}, nil, nil, nil, nil)
	next, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	m = next.(model)
