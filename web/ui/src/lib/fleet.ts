...
export type FleetEvent = FleetDelta | ProjectRemovedDelta
...
export function applyDeltaTo(rows: AgentRow[], d: FleetEvent): AgentRow[] { if (d.kind === 'project_removed') return rows.filter(r => r.project !== d.project); let changed = false; const out = rows.map(r => { if (r.id !== d.sessionId) return r; changed = true; if (d.kind === 'activity') return { ...r, activity: d.activity ?? r.activity }; if (d.kind === 'mode') return { ...r, mode: d.mode ?? r.mode }; if (d.kind === 'telemetry') return { ...r, contextPct: d.contextPct ?? r.contextPct, changedFiles: d.changedFiles ?? r.changedFiles }; return r }); return changed ? out : rows }
